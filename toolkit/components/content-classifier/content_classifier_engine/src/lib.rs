...
/// Request check for the browser blocker that also reports redirect and
/// rewrite directives. Kept separate from the function above so the existing
/// check can evolve without touching the blocker path.
#[no_mangle]
...
/// crate. Keep the FFI surface stable for the blocker service, but report no
/// native directives unless the vendored engine grows them.
#[no_mangle]
