...
WildBuzzard exposes one Pi-facing browser-control system. Its public tool
contract follows BrowserOS so models can use the established `tabs`,
...
session, and Pi must receive WildBuzzard page IDs and stable `eN` references
...
- BrowserOS browser-core and MCP tool implementations are the source of the
  ...
  in `extensions/browser-tools/NOTICE`.
- Mozilla Firefox DevTools MCP is an implementation and behavior reference for
  ...
  `extensions/browser-tools/MOZILLA-MCP-LICENSE`.

The BrowserOS core names remain the primary browsing API. Developer diagnostics
from Mozilla's MCP are added to the same Pi extension only where BrowserOS has
...
WildBuzzard page IDs, working-directory policy, authenticated loopback bridge,
and Gecko process. There is no second MCP server, selected-target state, or
