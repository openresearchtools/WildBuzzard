...
def install_packages(runner, artifacts, staging, allow_installed):
    ...
    runner.run(
        ["apt-get", "install", "-y", "--no-install-recommends", *paths],
        env=environment,
        timeout=1200,
    )
...
def validate_browser(runner, result_dir, account, environment):
    ...
    extension_uis = {
        ...
        "torrentSearch": validate_extension_ui(
            runner,
            result_dir,
            account,
            environment,
            ...
            page="src/popup.html",
            query="debian",
            query_name="Search torrents",
            screenshot_name="torrent-search-extension.png",
        ),
    }
