...
def verify_and_pull_base(recorder: Recorder, podman: list[str]) -> dict[str, object]:
    ...
    return {
        ...
        "applicationTreeReadOnlyMounted": True,
    }


def prepare_pristine_source_overlay(work: pathlib.Path) -> pathlib.Path:
    overlay = work / "pristine-source-overlay"
    shutil.copytree(SOURCE_ROOT / "upstream" / "searx", overlay, symlinks=True)
    shutil.copy2(HERE / "version_frozen.py", overlay / "version_frozen.py")
    return overlay
...
def main() -> int:
    ...
    try:
        ...
        pristine_source_overlay = prepare_pristine_source_overlay(work)
