...
The build verifies all 963 pristine source files, applies `patches/series`, checks all 616 effective providers against their reviewed source hashes, stages only eligible YAML, performs a locked self-contained .NET 9 publish in rootless Podman, rejects forbidden runtime paths, and writes deterministic runtime-manifest and SPDX files next to the output directory.
