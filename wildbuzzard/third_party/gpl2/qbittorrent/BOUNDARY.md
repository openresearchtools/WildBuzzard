...
The engine runs in the foreground with a lifetime pipe owned by the browser. Normal browser shutdown closes that pipe and waits for the engine to save its transfer state and exit. Browser crashes also close the pipe in the kernel, causing the engine to shut down. WildBuzzard mode rejects daemon startup and missing lifetime pipes. Downloads resume from saved state when the browser starts the engine again.
