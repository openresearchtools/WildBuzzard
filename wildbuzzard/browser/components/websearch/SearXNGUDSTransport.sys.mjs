...
function decodeChunked(source) {
  ...
  while (true) {
    ...
    if (
      ...
      throw new Error("SearXNG returned truncated chunked HTTP data");
    }
    result += source.slice(cursor, cursor + size);
    cursor += size + 2;
  }
}
...
export function parseSearXNGHTTPResponse(
  ...
  if (!statusMatch) {
    throw new Error("SearXNG returned invalid HTTP status");
  }
  ...
  for (const line of lines) {
    ...
    if (headers.has(name)) {
      throw new Error(`SearXNG returned duplicate ${name} header`);
    }
    headers.set(name, match[2].trim());
  }
  if ((headers.get("content-encoding") ?? "identity") !== "identity") {
    throw new Error("SearXNG returned unsupported content encoding");
  }
          ...
          if (!Components.isSuccessCode(status)) {
            finish(
              ...
              status === Cr.NS_ERROR_ABORT
                ? abortError()
                : new Error("SearXNG socket request failed")
            );
            return;
          }
          try {
...
export function requestSearXNGUDS(
  socket,
  {
    method = "GET",
    target,
  ...
  const bodySource = byteString(body);
  let request =
    ...
    "Connection: close\r\n" +
    "User-Agent: WildBuzzard-Search/1\r\n";
  if (body.length) {
