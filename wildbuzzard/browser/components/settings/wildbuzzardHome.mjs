...
function wrapHomepageNewTabs(config) {
  if (config._wildbuzzardCustomNewTab) {
    return;
  }
  config._wildbuzzardCustomNewTab = true;
  ...
  config.getControlConfig = (controlConfig, deps, setting) => {
    ...
    return {
      ...
      options: [
        ...
        {
          value: "custom",
          l10nId: "wildbuzzard-home-mode-choice-custom-new-tab-url",
        },
        ...rest,
      ],
    };
...
const CUSTOM_NEWTAB_ITEM = {
  ...
  l10nId: "wildbuzzard-home-new-tab-custom-url",
  controlAttrs: {
  ...
  items: [
    {
      ...
      l10nId: "wildbuzzard-home-new-tab-custom-url-input",
    },
  ],
};
...
function injectCustomNewTabItem(config) {
  ...
  if (
    config.items.some(item => item.id === "wildbuzzardCustomNewTabUrlGroup")
  ) {
    return;
  }
...
// WildBuzzard ships no sponsored content, so drop the "Support Firefox" sponsored
...
// Firefox brand; WildBuzzard uses neutral wording.
const HOME_LABEL_OVERRIDES = {
  "home-mode-choice-default-fx-srd": "wildbuzzard-home-mode-choice-default",
  "home-prefs-content-header": "wildbuzzard-home-prefs-content-header",
  "home-prefs-firefox-home-disabled-notice": "wildbuzzard-home-disabled-notice",
};
