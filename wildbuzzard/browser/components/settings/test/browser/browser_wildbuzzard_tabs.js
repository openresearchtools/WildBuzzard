...
add_task(async function test_tabs_group_renders() {
  ...
  let group = await settingGroupRenders(doc, "wildbuzzardTabs");
  ok(group, "The WildBuzzard tabs group renders on the tabs pane");

  let select = doc
    .getElementById("setting-control-wildbuzzard-tab-bar-position")
...
add_task(async function test_menu_toggle_writes_pref() {
  ...
  await settingGroupRenders(doc, "wildbuzzardTabs");
  let toggle = doc
    .getElementById("setting-control-wildbuzzard-tabs-duplicate-menu")
...
add_task(async function test_grouping_placement_follows_master() {
  ...
  await settingGroupRenders(doc, "wildbuzzardTabs");
  let placement = doc
    .getElementById("setting-control-wildbuzzard-auto-group-placement")
    ?.querySelector("moz-select");
  ...
  let toggle = doc
    .getElementById("setting-control-wildbuzzard-auto-group-tabs")
    ?.querySelector("moz-toggle");
  synthesizeClick(toggle);
  await prefChanged;
