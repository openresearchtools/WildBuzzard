...
const { WildBuzzardUpgradeMessage } = ChromeUtils.importESModule(
...
add_task(async function show_major_upgrade() {
  ...
  try {
    ...
    const data = await WildBuzzardUpgradeMessage.getUpgradeMessage();
    Assert.equal(
      data.id,
      "WILDBUZZARD_153_UPGRADE",
      "WildBuzzard 153 upgrade dialog shown"
    );
    Assert.equal(
      Services.prefs.getIntPref("browser.startup.upgradeDialog.version"),
      WildBuzzardUpgradeMessage.dialogVersion,
      "WildBuzzard upgrade dialog version was recorded"
    );
    win.close();
