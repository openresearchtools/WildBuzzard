...
const WIDGET_ID = "wildbuzzard-blocker-toolbar-button";
const PANEL_STYLESHEET_URI =
  "chrome://browser/content/blocker/wildbuzzardBlockerPanel.css";

const PANEL_IDS = {
  panel: "wildbuzzard-blocker-panel",
  multiview: "wildbuzzard-blocker-multiview",
...
/**
 * Owns the WildBuzzard blocker toolbar button and popup panel.
 *
 ...
 * and routes interactions to `WildBuzzardBlockerService`.
 */
export const WildBuzzardBlockerPanel = {
  _initialized: false,
  ...
  _handleTabOpen(win, event) {
    ...
    WildBuzzardBlockerService.recordNewTabSourceHost(
      newBrowserId,
  ...
  _hookBrowserWindow(win) {
    ...
    const onLocationChange = (
      ...
      if (isTopLevel && !isSameDocument) {
        ...
        if (browserId) {
          WildBuzzardBlockerService.resetBlockedCount(browserId);
        }
      }
  ...
  _setSiteExceptionForCurrentSite(win, disableForSite, sourceNode = null) {
    ...
    if (this._isPrivateWindow(win)) {
      if (disableForSite) {
        WildBuzzardBlockerService.allowSiteForSession(host, {
          isPrivate: true,
        });
      } else {
        WildBuzzardBlockerService.removeSiteException(host, {
          isPrivate: true,
        });
      }
    } else if (disableForSite) {
      WildBuzzardBlockerService.addSiteException(host);
    } else {
      WildBuzzardBlockerService.removeSiteException(host);
    }
  ...
  uninit() {
    ...
    for (const topic of OBSERVED_TOPICS) {
      try {
        ...
        console.warn(
          `[WildBuzzardBlockerPanel] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlockerPanel] Failed to remove pref observer:",
        err
      );
    }
