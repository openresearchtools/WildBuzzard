 ...
 */
export class WildBuzzardBlockedPageChild extends JSWindowActorChild {
  handleEvent(event) {
    ...
    this.sendQuery("WildBuzzardBlockedPage:AllowAndNavigate", {
      url: blockedUrl,
    })
      .then(ok => {
        if (!ok) {
      ...
      .catch(err => {
        console.error(
          "[WildBuzzardBlockedPageChild] AllowAndNavigate failed:",
          err
        );
      });
  }
