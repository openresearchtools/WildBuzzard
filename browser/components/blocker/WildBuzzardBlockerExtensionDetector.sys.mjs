...
ChromeUtils.defineLazyGetter(lazy, "blockerLocalization", () => {
  return new Localization(["browser/wildbuzzard/blocker.ftl"]);
});

const PREF_BLOCKER_ENABLED = "wildbuzzard.blocker.enabled";
const PREF_BLOCKER_UI_ENABLED = "wildbuzzard.blocker.ui.enabled";
...
const PREF_COEXIST = "wildbuzzard.blocker.coexist";

const DETECTION_DELAY_MS = 500;
const CHROME_DOCUMENT_LOADED_TOPIC = "chrome-document-loaded";
const PREF_CHANGED_TOPIC = "nsPref:changed";
const EXTENSION_NAME_PLACEHOLDER = "__WILDBUZZARD_BLOCKER_EXTENSION_NAME__";
const L10N_ID_EXTENSION_FALLBACK_NAME_THIS =
  "wildbuzzard-blocker-extension-fallback-name-this";
const L10N_ID_EXTENSION_FALLBACK_NAME_YOUR =
  "wildbuzzard-blocker-extension-fallback-name-your";
const L10N_ID_INSTALL_WARNING = "wildbuzzard-blocker-extension-install-warning";
const L10N_ID_INSTALL_WARNING_MANAGE_SETTINGS =
  "wildbuzzard-blocker-extension-install-manage-settings";
const L10N_ID_PROMPT_TITLE = "wildbuzzard-blocker-prompt-title";
const L10N_ID_INSTALL_ANYWAY = "wildbuzzard-blocker-extension-install-anyway";
 ...
 * warnings read from.
 */
export const WildBuzzardBlockerExtensionDetector = {
  _addonListener: {
    onEnabled(addon) {
      WildBuzzardBlockerExtensionDetector._onAddonStateChanged(addon);
    },
    onDisabled(addon) {
      WildBuzzardBlockerExtensionDetector._onAddonStateChanged(addon);
    },
  ...
  _installListener: {
    onInstallStarted(install) {
      return WildBuzzardBlockerExtensionDetector._onInstallStarted(install);
    },
    onInstallEnded(install) {
      WildBuzzardBlockerExtensionDetector._onInstallEnded(install);
    },
  },
  ...
  async _preloadLocalizedStrings() {
    ...
    const loadPromise = (async () => {
      ...
      this._localizedStringCache.set(
        "wildbuzzard-blocker-extension-install-warning-template",
  ...
  init() {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Failed to preload localized strings",
        error
      );
    });
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Startup detection failed",
        error
      );
    });
  },
  ...
  uninit() {
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlockerExtensionDetector] Failed to remove enabled observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlockerExtensionDetector] Failed to remove dismissed observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlockerExtensionDetector] Failed to remove coexist observer:",
        err
      );
    }
  },
  ...
  _onAddonStateChanged(addon) {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Failed to refresh detection notifications",
        error
      );
    });
  },
  ...
  _onBuiltInBlockerPrefChanged() {
    ...
    this._handleBuiltInBlockerReenabled()
      ...
      .catch(error => {
        console.error(
          "[WildBuzzardBlockerExtensionDetector] Re-enable handling failed",
          error
        );
      });
  },
  ...
  _onDetectionDismissedPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Failed to re-enable detection notifications",
        error
      );
    });
  },
  ...
  _onCoexistPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Failed to refresh after coexist toggle",
        error
      );
    });
  },
  ...
  _removeStartupObserver() {
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlockerExtensionDetector] Failed to remove startup observer:",
        err
      );
    }
    ...
    const progressListener = {
      onLocationChange: (browser, webProgress, _request, location) => {
        ...
        this._maybeShowDetectionUpgradeForBrowser(win, browser, location).catch(
          error => {
            console.error(
              "[WildBuzzardBlockerExtensionDetector] Failed to show upgrade modal",
              error
            );
          }
        );
  ...
  _seedDetectionForWindow(win, source = "seed") {
    ...
    const tryBrowser = browser => {
      this._maybeShowDetectionUpgradeForBrowser(
        ...
        console.error(
          `[WildBuzzardBlockerExtensionDetector] Failed to show ${source} upgrade modal`,
          error
        );
      });
    };
  ...
  async _prewarmUpgradeMessage() {
    ...
    this._upgradeMessagePromise =
      lazy.OnboardingMessageProvider.getUpgradeMessage()
        ...
        .catch(error => {
          console.error(
            "[WildBuzzardBlockerExtensionDetector] Failed to prewarm upgrade message",
            error
          );
          return null;
        })
  ...
  async _showDetectionUpgradeModal(win, browser, extensionName) {
    ...
    message.id = `WILDBUZZARD_BLOCKER_UPGRADE_${idSuffix}`;
    ...
    screen.id = `WILDBUZZARD_BLOCKER_UPGRADE_SCREEN_${idSuffix}`;

    const [spotlightTitle, spotlightPrimaryLabel, spotlightSecondaryLabel] =
      await lazy.blockerLocalization.formatValues([
        "wildbuzzard-blocker-spotlight-title",
        "wildbuzzard-blocker-spotlight-primary-button",
        "wildbuzzard-blocker-spotlight-secondary-button",
      ]);
    const detectedName =
      extensionName ||
    ...
    const spotlightSubtitle = await lazy.blockerLocalization.formatValue(
      "wildbuzzard-blocker-spotlight-subtitle",
  ...
  _onInstallStarted(install) {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Failed to refresh install warning strings",
        error
      );
    });
  ...
  async _disableAdblockExtensions(addons) {
    ...
    for (const addon of addons) {
      try {
        ...
        failed = true;
        console.error(
          `[WildBuzzardBlockerExtensionDetector] Failed to disable ${addon?.id || "<unknown>"}`,
          error
        );
      }
    }
  ...
  async _handleBuiltInBlockerReenabled() {
    ...
    const [promptTitle, messageText, useBuiltInLabel, keepExtensionLabel] =
      await lazy.blockerLocalization.formatValues([
        "wildbuzzard-blocker-prompt-title",
        {
          id: "wildbuzzard-blocker-reenable-conflict-message",
          args: { extensionName: conflictingName },
        },
        "wildbuzzard-blocker-reenable-use-built-in",
        "wildbuzzard-blocker-reenable-keep-extension",
      ]);
  ...
  _showInstallWarning(win, addon) {
    try {
      ...
      console.error(
        "[WildBuzzardBlockerExtensionDetector] Failed to show install warning prompt",
        error
      );
      return true;
    }
