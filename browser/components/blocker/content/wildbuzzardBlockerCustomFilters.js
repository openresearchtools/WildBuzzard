...
const CUSTOM_FILTERS_DEFAULT_FILE_NAME = "wildbuzzard-custom-filters.txt";
...
var gWildBuzzardBlockerCustomFilters = {
  _acceptButton: null,
  _dialog: null,
  _exportButton: null,
  _importButton: null,
  ...
  async init() {
    ...
    this._textarea = document.getElementById(
      "wildbuzzardBlockerCustomFiltersText"
    );
    this._status = document.getElementById(
      "wildbuzzardBlockerCustomFiltersStatus"
    );
    this._importButton = document.getElementById(
      "wildbuzzardBlockerCustomFiltersImport"
    );
    this._exportButton = document.getElementById(
      "wildbuzzardBlockerCustomFiltersExport"
    );
    ...
    this._importButton?.addEventListener("command", () => {
      this._importFromFile().catch(err => {
        console.error(
          "[WildBuzzardBlocker] Failed to import custom filters:",
          err
        );
        showAlert(
          "wildbuzzard-blocker-custom-filters-import-error-title",
          "Import failed",
          "wildbuzzard-blocker-custom-filters-import-error",
    ...
    this._exportButton?.addEventListener("command", () => {
      this._exportToFile().catch(err => {
        console.error(
          "[WildBuzzardBlocker] Failed to export custom filters:",
          err
        );
        showAlert(
          "wildbuzzard-blocker-custom-filters-export-error-title",
          "Export failed",
          "wildbuzzard-blocker-custom-filters-export-error",
    ...
    document.addEventListener("dialogaccept", event => {
      ...
      this._saveAndClose().catch(err => {
        console.error(
          "[WildBuzzardBlocker] Failed to save custom filters:",
          err
        );
        showAlert(
          "wildbuzzard-blocker-custom-filters-save-error-title",
          "Save failed",
          "wildbuzzard-blocker-custom-filters-save-error",
  ...
  async _load() {
    ...
    try {
      this._initialText =
        await WildBuzzardBlockerService.getCustomFiltersText();
      ...
      console.error("[WildBuzzardBlocker] Failed to load custom filters:", err);
      await showAlert(
        "wildbuzzard-blocker-custom-filters-load-error-title",
        "Load failed",
        "wildbuzzard-blocker-custom-filters-load-error",
  ...
  _updateStatus() {
    ...
    this._updateStatusLocalized(activeCount, dirty, generation).catch(err => {
      ...
      console.error(
        "[WildBuzzardBlocker] Failed to localize custom filters status:",
        err
      );
  ...
  async _updateStatusLocalized(activeCount, dirty, generation) {
    ...
    if (dirty) {
      const unsavedStatus = await formatValue(
        "wildbuzzard-blocker-custom-filters-status-unsaved",
        "Unsaved changes."
      );
  ...
  async _saveAndClose() {
    ...
    try {
      ...
      await WildBuzzardBlockerService.setCustomFiltersText(normalized);
  ...
  async _importFromFile() {
    const title = await formatValue(
      "wildbuzzard-blocker-custom-filters-import-picker-title",
    ...
    if (this._textarea.value !== this._initialText) {
      const confirmTitle = await formatValue(
        "wildbuzzard-blocker-custom-filters-import-replace-title",
        "Replace current custom filters?"
      );
      const confirmMessage = await formatValue(
        "wildbuzzard-blocker-custom-filters-import-replace-message",
  ...
  async _exportToFile() {
    const title = await formatValue(
      "wildbuzzard-blocker-custom-filters-export-picker-title",
...
document.addEventListener("DOMContentLoaded", () => {
  gWildBuzzardBlockerCustomFilters.init();
});
