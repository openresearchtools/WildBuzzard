...
const LIST_UPDATE_INTERVAL_MS = 24 * 60 * 60 * 1000;
...
const CATEGORY_L10N_IDS = Object.freeze({
  annoyances: "wildbuzzard-blocker-filter-lists-category-annoyances",
  core: "wildbuzzard-blocker-filter-lists-category-core",
  optional: "wildbuzzard-blocker-filter-lists-category-optional",
  privacy: "wildbuzzard-blocker-filter-lists-category-privacy",
  regional: "wildbuzzard-blocker-filter-lists-category-regional",
});
...
/**
 * Loads list metadata from `WildBuzzardBlockerService`, renders category sections
 * with toggles for each list, and saves state overrides when the dialog is
 * accepted.
 */
var gWildBuzzardBlockerFilterListsManager = {
  ...
  onLoad() {
    this._initialise().catch(err => {
      console.error(
        "[WildBuzzardBlocker] Failed to initialise filter list dialog:",
        err
      );
    });
  },
  ...
  async _loadEntries() {
    let catalog = [];
    try {
      catalog = await WildBuzzardBlockerService.getFilterListCatalog();
    } catch (err) {
      console.error("[WildBuzzardBlocker] Failed to load filter lists:", err);
    }
  ...
  _buildSections() {
    ...
    if (!this._entries.length) {
      ...
      setLabelL10nAttributes(
        emptyLabel,
        "wildbuzzard-blocker-filter-lists-empty-state"
      );
  ...
  _buildCategorySection(category, entries) {
    const container = createXULElement("vbox", {
      class: "wildbuzzard-blocker-category",
    });

    const header = createXULElement("hbox", {
      align: "center",
      "aria-expanded": "false",
      class: "wildbuzzard-blocker-category-header",
      role: "button",
      tabindex: "0",
    });
    ...
    const categoryLabel = createXULElement("label", {
      class: "wildbuzzard-blocker-category-title",
    });
    ...
    const counterLabel = createXULElement("label", {
      class: "text-deemphasized wildbuzzard-blocker-category-counter",
      value: "0/0",
    });
    ...
    const listContainer = createXULElement("vbox", {
      class: "wildbuzzard-blocker-category-lists",
    });
    ...
    for (const entry of entries) {
      const row = createXULElement("hbox", {
        align: "center",
        class: "wildbuzzard-blocker-list-row",
      });
      ...
      if (entry.sourceUrl) {
        const linkIcon = createXULElement("image", {
          class: "wildbuzzard-blocker-list-link-icon",
      ...
      const lastUpdatedLabel = createXULElement("label", {
        class: "text-deemphasized wildbuzzard-blocker-list-updated",
      });
  ...
  async _loadMetadata() {
    try {
      const metaList = await WildBuzzardBlockerService.getFilterListMetadata();
      return new Map(metaList.map(m => [m.url, m]));
    } catch (err) {
      console.error("[WildBuzzardBlocker] Failed to load metadata:", err);
      return new Map();
    }
  },
  ...
  _initSearch() {
    const input = document.getElementById(
      "wildbuzzardBlockerFilterListsSearch"
    );
  ...
  async _onRefreshNow() {
    const button = document.getElementById("wildbuzzardBlockerRefreshNow");
    button.disabled = true;

    try {
      await WildBuzzardBlockerService.refreshListsAndEngine();
      this._metadata = await this._loadMetadata();
      this._updateAllLastUpdatedLabels();
      this._updateNextRefreshLabel();
    } catch (err) {
      console.error("[WildBuzzardBlocker] Manual refresh failed:", err);
    } finally {
  ...
  _updateNextRefreshLabel() {
    const label = document.getElementById("wildbuzzardBlockerNextRefresh");
    if (!label) {
      return;
    }
      ...
      setLabelL10nAttributes(
        ...
        "wildbuzzard-blocker-filter-lists-next-refresh",
        {
          date: DATE_FORMATTER.format(
            new Date(anchorMs + LIST_UPDATE_INTERVAL_MS)
          ),
        }
      );
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  gWildBuzzardBlockerFilterListsManager.onLoad();
});

document.addEventListener("dialogaccept", event => {
  if (!gWildBuzzardBlockerFilterListsManager.onDialogAccept()) {
    event.preventDefault();
  }
});

document.addEventListener("dialogcancel", event => {
  if (!gWildBuzzardBlockerFilterListsManager.onDialogCancel()) {
    event.preventDefault();
  }
});
