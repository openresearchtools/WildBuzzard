...
const COSMETIC_STYLE_ID = "wildbuzzard-blocker-cosmetic-style";
 ...
 */
export class WildBuzzardBlockerChild extends JSWindowActorChild {
  /**
  ...
  receiveMessage(message) {
    try {
      switch (message.name) {
        case "WildBuzzardBlocker:ApplyCosmeticSelectors":
          return this._applyCosmeticSelectors(message.data?.selectors || []);

        case "WildBuzzardBlocker:ClearCosmeticSelectors":
          this._clearCosmeticSelectors();
          return { ok: true };

        case "WildBuzzardBlocker:CollectClassIdSnapshot":
      ...
      console.error("[WildBuzzardBlockerChild] receiveMessage failed:", err);
  ...
  _createEarlyScriptletBlocker() {
    ...
    const finish = (path = "aborted") => {
      ...
      if (Services.prefs.getBoolPref(EARLY_SCRIPTLET_DIAGNOSTICS_PREF, false)) {
        try {
          ...
          pageWindow.__wildbuzzardBlockerEarlyScriptletDiagnostics = {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlockerChild] failed to block parser for early scriptlet:",
        err
      );
      finish("block-error");
      return null;
  ...
  _initializeForCurrentDocument(eventName, earlyScriptletBlocker = null) {
    ...
    this._onDOMWindowCreated({ earlyScriptletBlocker })
      ...
      .catch(err => {
        console.error(
          `[WildBuzzardBlockerChild] ${eventName} initialization failed:`,
          err
        );
      })
      .finally(() => {
  ...
  async _onDOMWindowCreatedInner({ earlyScriptletBlocker = null } = {}) {
    ...
    try {
      ...
      console.error("[WildBuzzardBlockerChild] failed to get URL:", err);
      return;
    }
    ...
    let resources;
    try {
      resources = await this.sendQuery(
        "WildBuzzardBlocker:GetCosmeticResources",
        {
          url,
        }
      );
      ...
      console.error(
        "[WildBuzzardBlockerChild] sendQuery(GetCosmeticResources) failed:",
        err
      );
  ...
  didDestroy() {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlockerChild] failed clearing cosmetic selectors during destroy:",
        err
      );
    }
  }
  ...
  _clearResourceRetryTimeout() {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlockerChild] failed to clear resource retry timeout:",
        err
      );
    }
  ...
  _scheduleInitialResourcesRetry(targetDocument) {
    ...
    const timeoutId = this.contentWindow?.setTimeout(() => {
      ...
      this._retryInitialResources(targetDocument).catch(err => {
        console.error(
          "[WildBuzzardBlockerChild] delayed cosmetic resources retry failed:",
          err
        );
      });
  ...
  async _retryInitialResources(targetDocument) {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlockerChild] delayed sendQuery(GetCosmeticResources) failed:",
        err
      );
      return;
    }
  ...
  _flushInitialResources() {
    ...
    if (resources.injectedScript) {
      this._injectScriptlet(resources.injectedScript).catch(err => {
        console.error(
          "[WildBuzzardBlockerChild] failed to inject scriptlet:",
          err
        );
      });
    }
  ...
  _teardownProceduralActionObserver() {
    if (this._proceduralObserver) {
      try {
        ...
        console.error(
          "[WildBuzzardBlockerChild] failed to disconnect procedural observer:",
          err
        );
      }
    ...
    if (this._proceduralMutationTimeout) {
      try {
        ...
        console.error(
          "[WildBuzzardBlockerChild] failed to clear procedural timeout:",
          err
        );
      }
      this._proceduralMutationTimeout = null;
  ...
  async _queryAndApplyNewSelectors(classes, ids) {
    ...
    try {
      selectors = await this.sendQuery(
        "WildBuzzardBlocker:GetHiddenClassIdSelectors",
        {
          classes,
      ...
      console.error(
        "[WildBuzzardBlockerChild] sendQuery(GetHiddenClassIdSelectors) failed:",
        err
      );
      return;
    }
...
`;
    ...
    try {
      ...
      script = await ChromeUtils.compileScript(scriptUrl, {
        filename: "wildbuzzard-blocker-scriptlet.js",
      });
    } catch (err) {
      console.error(
        "[WildBuzzardBlockerChild] failed to compile scriptlet:",
        err
      );
      return false;
    }
  ...
  _teardownGenericHideObserver() {
    if (this._observer) {
      try {
        ...
        console.error(
          "[WildBuzzardBlockerChild] failed to disconnect generic-hide observer:",
          err
        );
      }
      this._observer = null;
    ...
    if (this._mutationTimeout) {
      try {
        ...
        console.error(
          "[WildBuzzardBlockerChild] failed to clear mutation timeout:",
          err
        );
      }
      this._mutationTimeout = null;
    ...
    if (this._initialCollectTimeout) {
      try {
        ...
        console.error(
          "[WildBuzzardBlockerChild] failed to clear initial collection timeout:",
          err
        );
      }
      this._initialCollectTimeout = null;
